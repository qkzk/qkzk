...
2. On suppose diposer de la variable `ville` en mémoire. Écrire une expression
...
3. On donne le code suivant : 
    ...
    a. Qeulle est la valeur de `le_plus_grand(pokemons)` ?
    b. Écrire le code d'une fonction `le_plus_leger` qui prend des Pokémons
        en paramètre et qui revoie un tuple dont la première composante est le 
        nom du Pokémon le plus léger et la deuxième est son poids.
